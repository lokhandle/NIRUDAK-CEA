...
# calculating mean DALYs lost conditional on dying; doesn't vary w/ sensitivity analysis

# TO DO: update with Japanese life tables, re-do in Stata 
# overtreat values are years of life lost given age (not DALY); undertreat is DALY which is fxnally the same thing
...
# if the probability of death from undertreatment is greater than or equal to 0.05 (this threshold might change with the
# age/life expectancy changes in Stata), NIRUDAK is more costly but more effective
...
################################################################################
# NEXT STEP: TWO WAY SENSITIVITY ANALYSIS (probability of death from overtreatment AND from undertreatment)

M_twoway_ICER <- M_twoway_inc_costs <- M_twoway_inc_DALYs_averted <- matrix(data=NA, 
...
colnames(M_twoway_ICER) <- colnames(M_twoway_inc_costs) <- 
  colnames(M_twoway_inc_DALYs_averted) <- paste("prdieOvr=", prob_die_gvn_overtreat_grid, sep = "")
